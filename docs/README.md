# Forage Documentation

## User Guides

- [Quick Start](quickstart.md) — Get an agent running in 2 minutes
- [Configuration Reference](config-reference.md) — Every config option explained

## Research & Design

Deep technical documentation on why Forage works the way it does:

- [Vision & Overview](research/00-VISION.md)
- [Prior Art](research/01-PRIOR-ART.md) — 15+ projects analyzed
- [Economics](research/02-ECONOMICS.md) — Budget, costs, tokenomics
- [Evolution Engine](research/03-EVOLUTION-ENGINE.md) — 16 biological mechanisms mapped to code
- [Architecture](research/04-ARCHITECTURE.md) — Full technical stack
- [Self-Improvement](research/05-SELF-IMPROVEMENT.md) — How agents learn
- [Biology Mapping](research/06-BIOLOGY-MAPPING.md) — Evolution → code
- [On-Chain](research/07-ONCHAIN.md) — Smart contract design (optional)
- [Safety](research/08-SAFETY.md) — Risks and guardrails
- [Build Plan](research/09-BUILD-PLAN.md) — Development roadmap
- [Open Source Model](research/10-OPEN-SOURCE.md) — Self-hosted architecture
- [Organism Layer](research/11-ORGANISM.md) — Drives, vitals, fitness-gated tiers, the species model
